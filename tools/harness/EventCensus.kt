...
fun main(args: Array<String>) {
    ...
    println()
    println("  The decision layer is live: the man on the ball generates concrete")
    println("  options, scores them on four axes and one is chosen by softmax. The")
    println("  random-strike placeholder is gone. So these rates are now produced by")
    println("  football rather than by noise — but the model is still missing")
    println("  tackles, fouls, offside, blocks, saves and the loose ball, so a rate")
    println("  that depends on any of those is structural, not a preference.")
    println()
    println("  Two known over-reads, named so nobody tunes against them:")
    println("    INTERCEPTION counts every change of hands, because every struck")
    println("      ball is claimed by somebody the moment it stops. There is no")
    println("      loose ball and no second ball, so an incomplete pass IS a")
    println("      turnover by construction. Against a 16-22 target it reads in the")
    println("      hundreds, and that gap is the missing loose-ball model, not a")
    println("      pressing figure anyone should adjust.")
    println("    Pass completion is depressed by the same thing.")
    println()
